...
app.controller('SystemCodeSortController',
    ['$scope', '$http', '$state', '$modal', '$stateParams', '$timeout', 'modalServ', 'BcCodeSortService', 'GG', '$rootScope',
        function ($scope, $http, $state, $modal, $stateParams, $timeout, modalServ, BcCodeSortService, GG, $rootScope) {
            ...
            $scope.isShow = false;
            $scope.OrgTreeDivIsOpened=false;
            $scope.showOrgTreeDiv = function () {
                $scope.isShow = true;
                $scope.OrgTreeDivIsOpened=true;
            }

            $scope.$watch('orgNo', function (newVal, oldVal) {
                if ($scope.orgNo != null && $scope.isShow == true) {
                    $scope.isShow = false;
                    $scope.loadCodeSort();
                }
            })
            ...
            $scope.loadCodeSort = function () {
                var tmp=null;
                if($scope.OrgTreeDivIsOpened){
                    tmp=$scope.orgNo;
                }
                BcCodeSortService.listCodeSort($scope.currentPage, $scope.keyword,tmp).then(function (res) {
                    console.log(res)
