...
app.controller('OrgController',function($scope,$http,$state,$modal,$timeout,modalServ,OrgService){
	...
	$scope.loadDepartmentList=function(){
		OrgService.loadDepartmentList($scope.currentPage,$scope.orgNo).then(function (res) {
			if (res.code=="10000"){
	 ...
	 * 修改部门
	 */
	$scope.update_department = function(org) {
		$scope.orgItem=org;
		if(org.orgType=="01"){
			$scope.operate_type="update_org";
		}else if(org.orgType=="02"){
			$scope.operate_type="update_department";
		}
		$scope.open("",$scope.operate_type,null);
	};

	/**
	 * 删除组织
	 */
	$scope.delete_department = function(org) {
